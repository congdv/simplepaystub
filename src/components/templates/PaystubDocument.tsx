...
const styles = StyleSheet.create({
  ...
  blueBar: { backgroundColor: '#7daafc', color: '#fff', flexDirection: 'row', justifyContent: 'space-between', padding: 8, marginBottom: 8 },
  tableHeader: { flexDirection: 'row', borderBottom: 1, borderColor: '#ccc', fontWeight: 'bold', marginBottom: 4 },
    ...
    <Page size="A4" orientation="landscape" style={styles.page}>
      ...
      <View style={styles.header}>
        <View>
          ...
          <Text>{data.payer.address || "123 Street st"}{" "}
            {data.payer.addressSecond ? "# " + data.payer.addressSecond : ""}</Text>
          <Text>{data.payer.city ? data.payer.city + ", " : "ABC, "}
            {data.payer.province || "YY"} {data.payer.postalCode || "XXX XXX"}</Text>
          <Text>{data.payer.phoneNumber || "(123) 456-7890"}
            {data.payer.extNo ? "# " + data.payer.extNo : ""}</Text>
          <Text>{data.payer.email}</Text>
        </View>
        <View style={{ alignItems: 'flex-end' }}>
          <Text style={styles.company}>{data.payee.name || "John Doe"}</Text>
          <Text>{data.payee.address || "123 Street st"}{" "}
            {data.payee.addressSecond ? "# " + data.payee.addressSecond : ""}</Text>
          <Text>{data.payee.city ? data.payee.city + ", " : "ABC, "}
            {data.payee.province || "YY"} {data.payee.postalCode || "XXX XXX"}</Text>
          <Text>{data.payee.phoneNumber || "(123) 456-7890"}{" "}
            {data.payee.extNo ? "# " + data.payee.extNo : ""}</Text>
      ...
      <View style={[styles.tableRow, styles.xPadded]}>
        <Text style={[styles.tableCol]}>{data.payment.type === DEFAULT_PAYMENT_TYPE ? "Hourly" : "Regular Salary"}</Text>
        <Text style={styles.tableCol}>{data.payment.type === DEFAULT_PAYMENT_TYPE &&
                data.payment.hourlyRate &&
                formatCurrency(Number(data.payment.hourlyRate))}</Text>
        <Text style={styles.tableCol}>{data.payment.type === DEFAULT_PAYMENT_TYPE &&
                data.payment.numOfHours}</Text>
      ...
      <View style={styles.blueTable}>
        ...
        <View style={styles.tableRow}>
            ...
            <Text style={styles.tableCol}>{formatCurrency(ytdTotalPayment - ytdTotalDeduction)}</Text>
            <Text style={styles.tableCol}></Text>
            <Text style={styles.tableCol}></Text>
            <Text style={styles.tableCol}></Text>
          </View>
