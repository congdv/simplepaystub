...
export default function NovaPayStubTemplate(data: PayStubType) {
  ...
  return (
    <div className="px-8 py-4 shadow-sm rounded-lg">
      ...
      <div className="mt-2">
        {data.payment.chequeNumber && <p>Check #: {data.payment.chequeNumber}</p>}
      </div>
      <div className="flex justify-between text-sm mt-2 bg-blue-400 p-2 text-white">
        ...
        {data.payment.date && (
          <p>Pay Date: {new Date(data.payment.date).toLocaleDateString('en-US')} </p>
        )}
      </div>
      <Table>
        <TableHeader>
        ...
        <TableBody>
          <TableRow>
            ...
            <TableCell className="text-right">{formatCurrency(regularPay)}</TableCell>
            <TableCell className="text-right">{formatCurrency(Number(data.payment.ytd))}</TableCell>
            <TableCell>{''}</TableCell>
          ...
          {Array(maxRows)
            ...
            .map((_, index) => (
              <TableRow key={index}>
                <TableCell className="font-medium">{data.benefits[index]?.label}</TableCell>
                <TableCell className="text-right"></TableCell>
                <TableCell className="text-right"></TableCell>
                <TableCell className="text-right">
        ...
        <TableFooter className="bg-blue-300">
          <TableRow>
            ...
            <TableCell className="text-right"></TableCell>
            <TableCell className="text-right"></TableCell>
            <TableCell className="text-right">{formatCurrency(payments)}</TableCell>
            <TableCell className="text-right">{formatCurrency(ytdTotalPayment)}</TableCell>
          ...
          <TableRow>
            ...
            <TableCell className="text-right"></TableCell>
            <TableCell className="text-right"></TableCell>
            <TableCell className="text-right">{formatCurrency(netPay)}</TableCell>
            <TableCell className="text-right">
